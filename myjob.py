...
def analyze_packet(packet):
    ...
    if IP in packet:
        ...
        # Обновляем счетчик пакетов и время последнего пакета
        source_ips[source_ip] += 1
        last_packet_time[source_ip] = time.time()

        # Проверяем, не превышает ли количество пакетов от данного IP-адреса за последнюю секунду заданный порог
        if source_ips[source_ip] > 10 and time.time() - last_packet_time.get(source_ip, 0) <= 1:
